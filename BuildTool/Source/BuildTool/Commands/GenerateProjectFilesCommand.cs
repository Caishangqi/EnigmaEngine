...
public sealed class GenerateProjectFilesCommand : ICommand
{
    ...
    public BuildResult Execute(BuildOptions options)
    {
        try
        {
            ...
            string buildToolCsproj = Path.GetFullPath(
                Path.Combine(engineRoot, "..", "BuildTool", "Source", "BuildTool", "BuildTool.csproj"));
            ...
            // 11. Collect rule files (.Build.cs and .Target.cs)
            string gameSourceDir = Path.Combine(projectRoot, "Source");
            var ruleFiles = CollectRuleFiles(engineRoot, projectRoot, gameSourceDir);
    ...
    /// <summary>Collect all .Build.cs and .Target.cs files for the Rules folder.</summary>
    private static List<string> CollectRuleFiles(string engineRoot, string projectRoot, string gameSourceDir)
    {
        var files = new List<string>();

        // Engine .Build.cs files
        string runtimeDir = Path.Combine(engineRoot, "Source", "Runtime");
        if (Directory.Exists(runtimeDir))
            files.AddRange(Directory.GetFiles(runtimeDir, "*.Build.cs", SearchOption.AllDirectories));

        // Engine .Target.cs files
        string engineSourceDir = Path.Combine(engineRoot, "Source");
        if (Directory.Exists(engineSourceDir))
            files.AddRange(Directory.GetFiles(engineSourceDir, "*.Target.cs", SearchOption.TopDirectoryOnly));

        // Game .Build.cs and .Target.cs files
        if (Directory.Exists(gameSourceDir))
        {
            files.AddRange(Directory.GetFiles(gameSourceDir, "*.Build.cs", SearchOption.AllDirectories));
            files.AddRange(Directory.GetFiles(gameSourceDir, "*.Target.cs", SearchOption.TopDirectoryOnly));
        }

        // Plugin .Build.cs files
        string pluginsDir = Path.Combine(projectRoot, "Plugins");
        if (Directory.Exists(pluginsDir))
            files.AddRange(Directory.GetFiles(pluginsDir, "*.Build.cs", SearchOption.AllDirectories));
