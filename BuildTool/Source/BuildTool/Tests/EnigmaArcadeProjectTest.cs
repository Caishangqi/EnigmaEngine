...
public static class EnigmaArcadeProjectTest
{
    ...
    private static void TestJsonIntegration(string projectRoot)
    {
        ...
        // [36] Output format matches REQ-014: [EnigmaArcade] Config: {"game":"EnigmaArcade","version":1}
        Check(gameInstanceCpp.Contains("[EnigmaArcade] Config: ")
              && gameInstanceCpp.Contains("config.dump()"),
            "[36] Output format: [EnigmaArcade] Config: <json> (REQ-014)");
    }
    ...
    private static void TestRuntimeOutputFormat(string projectRoot)
    {
        ...
        var gameInstanceCpp = File.ReadAllText(
            Path.Combine(projectRoot, "Source", "EnigmaArcade", "Private", "ArcadeGameInstance.cpp"));
        Check(gameInstanceCpp.Contains("[EnigmaArcade] Engine Initialized"),
            "[49] FArcadeGameInstance outputs '[EnigmaArcade] Engine Initialized'");
        ...
        // [52] Tick counter output
        Check(gameInstanceCpp.Contains("[EnigmaArcade] Tick: ")
              && gameInstanceCpp.Contains("TickCount"),
            "[52] Update outputs '[EnigmaArcade] Tick: N' with incrementing counter");
    }
    ...
    private static string ResolveProjectRoot()
    {
        ...
        // Navigate from build output to Games/EnigmaArcade
        var candidate = Path.GetFullPath(Path.Combine(
            baseDir, "..", "..", "..", "..", "..", "Games", "EnigmaArcade"));
        ...
        while (dir is not null)
        {
            var testDir = Path.Combine(dir.FullName, "Games", "EnigmaArcade");
            if (Directory.Exists(testDir))
                return testDir;
            dir = dir.Parent;
        }

        throw new DirectoryNotFoundException("Cannot find Games/EnigmaArcade project directory.");
    }
