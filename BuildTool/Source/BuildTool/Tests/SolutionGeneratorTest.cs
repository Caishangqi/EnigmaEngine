...
public static class SolutionGeneratorTest
{
    ...
    private static (string tempDir, SolutionGenerator.GenerateInput input) CreateTestInput()
    {
        ...
        {
            ["EnigmaArcade"] = new() { ModuleName = "EnigmaArcade", ModuleDirectory = Path.Combine(tempDir, "Games", "EnigmaArcade", "Source", "EnigmaArcade") },
            ["ArcadeGameplay"] = new() { ModuleName = "ArcadeGameplay", ModuleDirectory = Path.Combine(tempDir, "Games", "EnigmaArcade", "Source", "ArcadeGameplay") },
        };

        var pluginModules = new Dictionary<string, ModuleRules>
        {
            ["ArcadeFeature"] = new() { ModuleName = "ArcadeFeature", ModuleDirectory = Path.Combine(tempDir, "Games", "EnigmaArcade", "Plugins", "ArcadeFeature", "Source", "ArcadeFeature") },
        };
    ...
    {
        Console.WriteLine("[Test 2] Solution folders: Engine, Games, Plugins, Programs, Rules");
        var (tempDir, input) = CreateTestInput();
        try
        {
            var result = new SolutionGenerator().Generate(input);
            ...
            Assert(content.Contains($"Project(\"{folderGuid}\") = \"Rules\""), "Missing Rules folder");
            Console.WriteLine("  PASSED");
        }
        finally { Cleanup(tempDir); }
    }
    ...
    private static void TestNestedProjects()
    {
        ...
        try
        {
            ...
            string coreGuid = $"{{{GuidGenerator.GenerateForProject("Core").ToString().ToUpperInvariant()}}}";
            string engineSrcGuid = $"{{{GuidGenerator.GenerateForFolder("Engine/Source").ToString().ToUpperInvariant()}}}";
            Assert(content.Contains($"{coreGuid} = {engineSrcGuid}"), "Core should be nested under Engine/Source");
    ...
    private static void TestThirdPartyFolder()
    {
        ...
        try
        {
            ...
            string thirdPartyGuid = $"{{{GuidGenerator.GenerateForFolder("Engine/ThirdParty").ToString().ToUpperInvariant()}}}";
