...
// ---------------------------------------------------------------
class HotReloadShadowCopyTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ---------------------------------------------------------------
TEST_F(HotReloadShadowCopyTest, ShadowCopy_CreateAndCleanup)
{
    ...
    EXPECT_TRUE(fs::exists(ShadowPath));
    ...
    // Verify content matches.
    {
        ...
        EXPECT_TRUE(Content.find("FAKE_DLL_CONTENT") != std::string::npos);
    }

    // Cleanup.
    fs::remove(ShadowPath, Ec);
    EXPECT_FALSE(fs::exists(ShadowPath));
}

// ---------------------------------------------------------------
// ShadowCopy_PdbCopiedAlongside: verify PDB is copied too.
// ---------------------------------------------------------------
TEST_F(HotReloadShadowCopyTest, ShadowCopy_PdbCopiedAlongside)
{
    auto DllPath = CreateFakeDll("EnigmaEngine-TestModule");
    ...
    EXPECT_TRUE(fs::exists(ShadowDll));
    EXPECT_TRUE(fs::exists(ShadowPdb));

    // Cleanup both.
    fs::remove(ShadowDll, Ec);
    fs::remove(ShadowPdb, Ec);
    EXPECT_FALSE(fs::exists(ShadowDll));
    EXPECT_FALSE(fs::exists(ShadowPdb));
}

// ---------------------------------------------------------------
// ShadowCopy_VersionIncrement: multiple copies get unique names.
// ---------------------------------------------------------------
TEST_F(HotReloadShadowCopyTest, ShadowCopy_VersionIncrement)
{
    auto DllPath = CreateFakeDll("EnigmaEngine-TestModule");
    std::string ModuleName = "TestModule";
    ...
    EXPECT_TRUE(fs::exists(TempDir / "TestModule-Live-1.dll"));
    EXPECT_TRUE(fs::exists(TempDir / "TestModule-Live-2.dll"));
    EXPECT_TRUE(fs::exists(TempDir / "TestModule-Live-3.dll"));
}

// ---------------------------------------------------------------
// Event delegate tests using TMulticastDelegate directly.
// ---------------------------------------------------------------
class HotReloadEventTest : public ::testing::Test
{
protected:
...
// ---------------------------------------------------------------
TEST_F(HotReloadEventTest, OnModuleReloaded_Broadcast)
{
    ...
    EXPECT_EQ(ReceivedModule, "TestModule");
...
// ---------------------------------------------------------------
TEST_F(HotReloadEventTest, OnReloadFailed_Broadcast)
{
    std::string ReceivedModule;
    ...
    EXPECT_EQ(ReceivedModule, "TestModule");
    EXPECT_EQ(ReceivedError, "DLL load failed");
...
// ---------------------------------------------------------------
TEST_F(HotReloadEventTest, NonReloadable_Rejection)
{
...
// ---------------------------------------------------------------
TEST(HotReloadDebounceTest, Debounce_BatchChanges)
{
    ...
    EXPECT_LT(CurrentTime - LatestDetection, DebounceDelay);

    // Advance past debounce.
    CurrentTime = 1.8;
    EXPECT_GE(CurrentTime - LatestDetection, DebounceDelay);
    ...
    EXPECT_EQ(Unique.size(), 2u);  // ModuleA and ModuleB
    EXPECT_TRUE(Unique.contains("ModuleA"));
    EXPECT_TRUE(Unique.contains("ModuleB"));
}
