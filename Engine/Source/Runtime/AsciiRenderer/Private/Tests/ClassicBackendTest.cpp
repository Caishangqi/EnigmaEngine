...
class ClassicBackendColorTest : public ::Enigma::FAutomationTestFixture
{
protected:
...
ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactBlack)
{
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(0, 0, 0)), 0);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactWhite)
{
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(255, 255, 255)), 15);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactRed)
{
	// Palette index 12 = 0x000000FF -> R=255, G=0, B=0
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(255, 0, 0)), 12);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactGreen)
{
	// Palette index 10 = 0x0000FF00 -> R=0, G=255, B=0
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(0, 255, 0)), 10);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactBlue)
{
	// Palette index 9 = 0x00FF0000 -> R=0, G=0, B=255
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(0, 0, 255)), 9);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactYellow)
{
	// Palette index 14 = 0x0000FFFF -> R=255, G=255, B=0
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(255, 255, 0)), 14);
}

ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, ExactGray)
{
	// Palette index 7 = 0x00C0C0C0 -> R=192, G=192, B=192
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(192, 192, 192)), 7);
}

// Near-match: slightly off-white should still map to White (15).
ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, NearWhite)
{
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(250, 250, 250)), 15);
}

// Near-match: dark red (128,0,0) -> palette index 4 (Dark Red = 0x00000080 -> R=128)
ENIGMA_IMPLEMENT_ASCII_RENDERER_AUTOMATION_TEST_F(ClassicBackendColorTest, DarkRed)
{
	TestEqual("EXPECT_EQ", MapColorToIndex(FColor(128, 0, 0)), 4);
}
