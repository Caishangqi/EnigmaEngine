...
public sealed class VcxprojGenerator
{
    ...
    public sealed class ModuleProjectInput
    {
        ...
        /// <summary>Optional path to the module's .Build.cs file, included as a &lt;None&gt; item.</summary>
        public string? BuildCsPath { get; init; }

        /// <summary>Optional paths to .Target.cs files, included as &lt;None&gt; items at root level.</summary>
        public IReadOnlyList<string> TargetCsPaths { get; init; } = [];
    }
    ...
    private static string GenerateVcxproj(ModuleProjectInput input)
    {
        ...
        // Build.cs and Target.cs as None items
        var noneItems = new List<string>();
        if (input.BuildCsPath is not null)
            noneItems.Add(input.BuildCsPath);
        foreach (var targetCs in input.TargetCsPaths)
            noneItems.Add(targetCs);
        ...
        if (noneItems.Count > 0)
        {
            ...
            {
                string relativePath = GetRelativePath(input.OutputDirectory, item);
    ...
    private static string GenerateFilters(ModuleProjectInput input)
    {
        ...
        // Build.cs and Target.cs as None (root level, no filter)
        var noneItems = new List<string>();
        if (input.BuildCsPath is not null)
            noneItems.Add(input.BuildCsPath);
        foreach (var targetCs in input.TargetCsPaths)
